...
renderer, Guard, or the event schema  -  but the new art has to keep four things,
...
never find out. `node test/run-tests.js` has a test that fails if Guard emits
...
## Working on it without running anything else
    ...
    cd pet && npm run start:demo     # replays a canned sequence, no Guard needed
